...
> SRS** (реальный эфир, распознавание подтверждено) ✅ · **M3 — стриминг/качество** (partial-субтитры,
> стики-ярлык спикера, докатка текста, авто-реконнект ffmpeg) ✅ · интеграция в панель BondcastStream
> (`server/stream/panel`) — список активных стримов, кнопки «Собрать»/«Подключить субтитры» ✅ ·
> M2 (тюнинг порога speaker_threshold на реальных голосах) впереди.
...
- `Dockerfile`, `docker-compose.yml` (standalone, для изолированной разработки/тестов),
  ...
  с панелью — `speaker_enabled: false`, без энроллмента по умолчанию), `README.md`.

`server/stream/panel/` — управление: `GET /api/streams` (что сейчас реально льётся в SRS),
`POST /api/captions/build` (ленивая сборка GPU-образа), `POST /api/captions/connect|disconnect`
(пересоздание `asr-worker` под конкретный стрим), `GET /api/captions/status`.
